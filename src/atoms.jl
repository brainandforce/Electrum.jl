...
function supercell(l::AtomList{D}, M::AbstractMatrix{<:Integer}) where D
    ...
    # Right divide atomic coordinates by the supercell matrix to get new positions
    tmplist = [
        AtomPosition(
            atomname(atom),
            atomicno(atom),
            (v = M \ coord(atom);  v - floor.(v)) # Keep the new atoms inside the supercell
        )
        for atom in l.coord
    ]
    # Use LU decomposition to generate the translation bounds
    # Convert to a positive integer to generate valid indices
    tmax = Int.(abs.(diag(lu(M).U)))
