...
function readHGH(io::IO)
    ...
    # Get zatom and zion
    (zatom, zion) = parse.(Int8, split(ln)[1:2])
    # Get the exchange-correlation functional used and maximum l
    (pspcod, pspxc, lmax) = parse.(Int8, split(readline(io))[1:3])
    # pspcod should *always* equal 3 for valid HGH pseudopotentials
    if pspcod != 3
        error("pspcod was not 3")
    end
    (rloc, c1, c2, c3, c4) = parse.(Float64, split(readline(io))[1:5])
